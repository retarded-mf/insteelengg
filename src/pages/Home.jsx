            ...
            {[
              ...
              <div key={i} className="reveal-on-scroll group">
                <div className="h-[500px] overflow-hidden mb-10 relative">
                  <EditImage id={`home_featured_project_${i}_image`} defaultUrl={proj.img} alt={proj.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-1000" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
                <h3 className="text-charcoal font-black text-3xl mb-2 uppercase tracking-tighter">
                  <EditText id={`home_featured_project_${i}_name`} defaultValue={proj.name} />
                </h3>
                <div className="text-primary-red text-[11px] font-black uppercase tracking-[0.3em] mb-4">
                  <EditText id={`home_featured_project_${i}_type`} defaultValue={proj.type} />
                </div>
                <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">
                  <EditText id={`home_featured_project_${i}_loc`} defaultValue={proj.loc} /> — Site Archive
                </p>
              </div>
            ))}
          </div>
            ...
            {[
              ...
              <div key={i} className={`flex flex-col ${prin.rev ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-32 reveal-on-scroll`}>
                <div className="w-full lg:w-3/5 relative group">
                  ...
                  <EditImage id={`home_principle_${i}_image`} defaultUrl={prin.img} alt={prin.title} className="w-full h-[600px] object-cover shadow-2xl" />
                </div>
                <div className="w-full lg:w-2/5">
                  <span className="text-primary-red font-black text-xs uppercase tracking-[0.6em] mb-8 block opacity-50">
                    /<EditText id={`home_principle_${i}_tag`} defaultValue={prin.tag} />/
                  </span>
                  <h3 className="text-6xl md:text-8xl font-black text-charcoal mb-10 uppercase tracking-tighter leading-none">
                    <EditText id={`home_principle_${i}_title`} defaultValue={prin.title} />
                  </h3>
                  <p className="text-2xl text-gray-400 leading-relaxed font-bold italic border-l-2 border-gray-100 pl-10">
                    <EditText id={`home_principle_${i}_body`} defaultValue={prin.body} isTextArea={true} />
                  </p>
                </div>
              </div>
            ))}
